...
- **Bus adapters** (`adapters/*_bus.py`) — a *separate* set of adapters for swarm HITL approvals (Telegram/Discord/Slack). See [Security & Safety](security-and-safety.md#the-swarm-bus-gate).
...
The full SSE event contract (`token`, `tool_call`, `tool_result`, `approval_required`, `done`, `error`) is documented in [API & Extension Points](api-and-extension-points.md#sse-event-contract).
...
> The **bus adapter singleton** priority is Telegram > Discord > Slack (only one active at a time, wired in `app.py:506-556`). See [Security & Safety](security-and-safety.md#bus-adapter-priority).
