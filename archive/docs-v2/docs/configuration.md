...
| `memory.retrieval_top_k` | int | `5` | Top-K for memory retrieval (used by compaction's intended retrieval step — see [Memory & RAG](memory-and-rag.md)). |
...
> The **swarm bus** uses a separate, broader list (`_EXTENDED_DANGER` adds `python_exec`, `code_exec`, `spawn_agent`, `spawn_agents`, `schedule_task`, `cancel_scheduled`, `run_tests`). The **MCP** path classifies dynamically by name pattern. See [Security & Safety → danger-tool lists](security-and-safety.md#danger-tool-lists-three-of-them).
...
> These files declare a security **policy posture**. Whether every check is actively enforced at runtime should be verified against the hardening runner before relying on it in production — see [Security & Safety](security-and-safety.md).
