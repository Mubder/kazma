...
def _synthesize_refined_output(
    task: str,
    stages: list[Any],
    overall_status: str,
    total_ms: float,
) -> str:
    """Build a clean Markdown report card from pipeline stage outputs.

    This is the Refiner / Middleman function — it captures the raw
    outputs from all workers, strips noise, and produces a formatted
    summary card for delivery to the triggering platform.
    """
    lines: list[str] = []
    lines.append("## Swarm Report")
    lines.append("")
    lines.append(f"**Task:** {task[:200]}")
    lines.append(f"**Status:** {'✅ Completed' if overall_status == 'completed' else '❌ ' + overall_status.title()}")
    lines.append(f"**Duration:** {total_ms:.0f}ms")
    lines.append("")

    completed_count = sum(1 for s in stages if getattr(s, "status", "") == "completed")
    if completed_count > 1:
        lines.append("---")
        lines.append("### Stage Outputs")
        lines.append("")
        for stage in stages:
            status_icon = "✅" if getattr(stage, "status", "") == "completed" else "❌"
            role = str(getattr(stage, "role", "?"))
            output = getattr(stage, "output", "") or ""
            truncated = output[:500] + ("…" if len(output) > 500 else "")
            lines.append(f"{status_icon} **{role}** ({getattr(stage, 'duration_ms', 0):.0f}ms):")
            lines.append(f"> {truncated}")
            lines.append("")

    # Single synthesized summary
    all_outputs = [getattr(s, "output", "") or "" for s in stages if getattr(s, "status", "") == "completed"]
    if all_outputs:
        combined = " ".join(o.strip() for o in all_outputs if o.strip())
        lines.append("---")
    ...
    lines.append("---")
    lines.append(f"*Generated by Kazma Swarm — {len(stages)} stage(s), {completed_count} completed*")
    return "\n".join(lines)
...
class PipelineEngine:
    ...
    async def execute(
        ...
        refined_output = _synthesize_refined_output(task, stages, overall_status, total_ms)
