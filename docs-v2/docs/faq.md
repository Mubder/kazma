...
Ten presets ship: OpenAI, Anthropic, DeepSeek, Google Gemini (ADC), xAI, OpenRouter, Ollama, LM Studio, NVIDIA NIM, Custom. Any OpenAI-compatible endpoint works. See [Configuration → provider presets](configuration.md#52-built-in-provider-presets).
...
Memory retrieval is **not automatic**. The agent only retrieves when it calls `memory_search`. There's no injection and no short-term→permanent consolidation. See [Memory & RAG → Honest status](memory-and-rag.md#7-honest-status-notes-read-before-relying-on-memory-features).
...
Three things to check (see [Troubleshooting §3](troubleshooting-and-workarounds.md#3-hitl--safety-issues)):
...
Yes. `kazma hub sign` writes an HMAC-SHA256 signature; the loader verifies it fail-closed with `hmac.compare_digest`. See [Skills, MCP & Tools](skills-mcp-and-tools.md#33-cryptographic-signing-hmac-sha256--verified).
...
See [CLI Reference → swarm](cli-reference.md#6-kazma-swarm).
...
No. Metrics are in-memory + SQLite. See [Roadmap](roadmap-and-future.md#8-observability).
...
Docker Compose is the primary path. Bare `kazma serve` works for single-host dev. See [Deployment](deployment.md).
...
They deploy a **Hub API** service (PostgreSQL + Redis), not the main agent. Don't apply them for the main agent. See [Deployment §4](deployment.md#4-kubernetes-hub-service-only--read-carefully).
...
WAL + `busy_timeout=5000` is set everywhere. You likely have a long write transaction or a second connection. Use `batch_set()` for multi-key writes and always use `get_config_store()`. See [Troubleshooting §4](troubleshooting-and-workarounds.md#4-sqlite-concurrency).
