...
> **PowerShell rule (from AGENTS.md):** never chain commands with `&&` or `||`. Use `;` and check `$LASTEXITCODE`. The Bash tool in this environment uses Git Bash, not PowerShell.

---

## 6. Server management (from AGENTS.md)
...
# Start (background), dev port 8090
