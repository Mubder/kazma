...
- **TelegramWorker external dependency:** `TelegramWorker` shells out to `hermes -p <profile>`. `hermes` is not bundled with Kazma. If not installed, Telegram workers return errors. Prefer `InProcessWorker` for environments without `hermes`.
