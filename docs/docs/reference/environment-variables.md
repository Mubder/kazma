...
| `KAZMA_CHILD_ENV_ALLOW` | unset | Names (comma-separated) let through the secret filter on the processes tools start — pytest, `pip`/`npm` installs, `ruff`, git (`security/child_env.py`). Those children run a repository's tests and hooks and packages' install scripts; by default they get the server's environment without any `KAZMA_*` variable, credential-named variable (`*_TOKEN`, `*_API_KEY`, `*_PASSWORD`, …) or URL carrying a password. A listed name reaches every such child. `KAZMA_*` names are never let through. | One name a child genuinely needs, such as a private npm registry's `NPM_TOKEN`. |
