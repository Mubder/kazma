...
| Chats freeze 15–70 s; guard restarts with `unhealthy (probe error: timed out)` | `.kazma/stall-*.txt` — the event-loop thread's top Kazma frame names a sync helper doing I/O from async code | Add it to `_LOOP_STALL_HELPERS` (`tests/test_static_gates.py`); wrap every async call site in `asyncio.to_thread` | AGENTS §35; weekly report "event loop stall" |
| Health probes fail with `WinError 10048` / `10055` | The **machine** is out of ports, not Kazma: `guard.log` `health.probe_unrunnable` + `health.port_exhaustion` (socket owners) | Windows System log, Tcpip 4231 / 4227 / 4266; widen the range (`netsh int ipv4 set dynamicport tcp start=10000 num=55535`) | Guard — these probes never count toward a restart |
| Weekly resilience report says a mechanism is "silent" | Did the code log a **success** line? Does the signature match it (`test_every_ledger_signature_matches_a_line_the_code_emits`)? | The report reads rotated logs; search `kazma.log.*` for the line yourself | AGENTS §27C |
| Agent "forgot" an old conversation | `episodes.tier = 'archived'` — only the `summary_text` stub remains | Archival needs created **and** unrecalled past `episodic_ttl_days`; restore text from a backup taken before the archive date | §9 Memory; `memory-and-rag` "Episode lifecycle" |
| Backup repository keeps growing | `restic forget` grouping — must be `--group-by host,tags` | `[restic] maintenance ok` in the log? | `ops/disaster-recovery` Retention |
