...
|------|--------------|-------|
| **Chat as the product (2026-08-26)** | `/` is immersive (no Chat/Home/Chat header). Composer: attach + input + send; Long/YOLO/cost live under **⋯**. Sidebar: Chat / Workspace / IDE, inspectors under **More**. Dashboard is an inspector, not the hero. | [GOAL](https://github.com/Mubder/kazma/blob/main/docs/plans/CHAT_AS_PRODUCT_UI_GOAL.md); restart after pull |
