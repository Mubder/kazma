...
**Check 1 — is the graph gate active?** All production build sites pass `hitl_config` (see [Security & Safety §2.2](security-and-safety)). If you wrote a custom build via `create_supervisor_graph()` without `hitl_config`, the gate is **dormant**.
...
Never use `&&` or `||` in PowerShell. Use `;` and check `$LASTEXITCODE` (per AGENTS):
