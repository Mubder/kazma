...
| Division checks do nothing unless `KAZMA_DIVISION` is set | Incorrect as written. `division_runtime.py:113` also supports `KAZMA_DIVISION_ENFORCE`; `:122` falls back to ConfigStore `agent.division`. Default unconfigured posture remains unenforced. |
